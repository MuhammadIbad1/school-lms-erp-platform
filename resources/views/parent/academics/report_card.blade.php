...
@section('title', $child->user->name . ' - Report Card')
@section('header', 'Official Student Report Card')
...
<div class="space-y-6">
    ...
    <div class="glass-card p-6 flex flex-wrap items-center justify-between gap-4 no-print">
        <form method="GET" action="{{ route('parent.report-card') }}" class="flex items-center space-x-3">
    ...
    <div class="glass-card p-8 md:p-12 print-full-width border border-slate-200 bg-white">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 p-4 rounded-2xl bg-slate-50 border border-slate-200 mb-8 text-xs">
            <div>
                ...
                <p class="font-extrabold text-slate-900 mt-0.5">{{ $child->user->name }}</p>
            </div>
            <div>
                <p class="text-slate-400 font-bold uppercase text-[10px]">Admission #</p>
                <p class="font-mono font-bold text-indigo-600 mt-0.5">{{ $child->admission_number }}</p>
            </div>
            <div>
                <p class="text-slate-400 font-bold uppercase text-[10px]">Class & Section</p>
                <p class="font-bold text-slate-800 mt-0.5">{{ $child->schoolClass->name }} - {{ $child->section->name }}</p>
            </div>
            <div>
                <p class="text-slate-400 font-bold uppercase text-[10px]">Roll Number</p>
                <p class="font-bold text-slate-800 mt-0.5">{{ $child->roll_number }}</p>
            </div>
        </div>
